...
type Platform interface {
	...
	// InstallAgent downloads the kontango agent binary and installs it as a
	// persistent service that runs alongside the tunnel.
	// baseURL is the controller enroll URL (e.g. https://your-controller.example).
	InstallAgent(baseURL, identityFile string) error
	// InstallCaddy downloads the Ziti-enabled Caddy binary and installs it
	// as a persistent egress proxy service. The Caddyfile is written to
	// KontangoDir()/Caddyfile and can be extended by the user.
	InstallCaddy(baseURL, identityFile string) error
	StartService() error
	EnsureDir() error
