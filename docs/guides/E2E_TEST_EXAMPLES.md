...
To test against the actual Kontango controller at `join.kontango.net`:

```bash
# Enroll as new machine
./build/kontango enroll https://join.kontango.net:1280 --no-tui

# Re-enroll known machine
./build/kontango enroll https://join.kontango.net:1280 --scan --no-tui

# Enroll with AppRole (pre-provisioned)
./build/kontango enroll https://join.kontango.net:1280 \
