...
|-----------|---------|---------|
| `*.prod.example.com` | localhost:1280 | Ziti controller (TLS passthrough) |
| `bao.*` | localhost:8200 | Bao API (TLS passthrough) |
| All other TLS | localhost:10443 | Caddy HTTPS handler (TLS termination) |

This requires:
1. Router configs updated to use `ctrl-N.prod.example.com:443` as the controller endpoint
2. Controller advertise address changed to `ctrl-N.prod.example.com:443`
