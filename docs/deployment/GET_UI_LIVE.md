...
### Desired State
1. User visits `https://join.kontango.net/` → sees enrollment web UI
...
curl https://join.kontango.net/ | grep -c "Welcome to TangoKore"
# Should output: 1

# Test installer endpoint
curl https://join.kontango.net/install | head -3
# Should return: #!/bin/bash

# Test CSS loads
curl https://join.kontango.net/css/style.css | head -5
# Should return: CSS content

# Test JavaScript loads
curl https://join.kontango.net/js/main.js | head -5
...
```bash
# 1. Visit web UI
curl https://join.kontango.net/ | head -20
# Should see HTML with "Welcome to TangoKore"

# 2. Get installer script
curl https://join.kontango.net/install | head -10
# Should return:
# #!/bin/bash
# export BASE_URL="https://join.kontango.net"
...
1. **Open** `https://join.kontango.net/`
...
### After Deployment
- [ ] Test public endpoint (`https://join.kontango.net/`)
- [ ] HTML loads
- [ ] CSS loads
- [ ] JavaScript loads
...
✅ `https://join.kontango.net/` returns HTML (enrollment UI)  
...
✅ `https://join.kontango.net/install` returns shell script  
