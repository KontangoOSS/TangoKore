...
**Impact:** Users cannot run `curl https://join.kontango.net/install | sudo sh`

## What Needs to Happen

The join endpoint should:
1. **Serve installer script** at `https://join.kontango.net/install`
2. **Serve join webpage** at `https://join.kontango.net/`
...
curl -k https://join.kontango.net/ -v  # Check headers for routing
```
...
# Test publicly
curl -k https://join.kontango.net/install | head -3
```
...
- Certificate: Must cover `join.kontango.net`
- Currently: Likely have wildcard for `.kontango.net`
- Issue: Confirm cert is valid and installed

```bash
# Check certificate on controller
openssl s_client -connect join.kontango.net:443
```

### 5. DNS Configuration

- **Hostname:** `join.kontango.net`
...
✅ `curl https://join.kontango.net/install` returns shell script  
✅ `curl https://join.kontango.net/` returns join webpage HTML  
