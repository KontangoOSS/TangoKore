...
- **One command to join** — `curl https://join.kontango.net/install | sudo sh`
...
- Integration tests with real join.kontango.net endpoint
...
**Location:** `web/public/` + deployed at `join.kontango.net`
...
- **Web interface** (join.kontango.net)
...
```
User runs:  curl https://join.kontango.net/install | sudo sh
            ↓
...
### Join Endpoint
- **URL:** `https://join.kontango.net/install`
...
# Test with join.kontango.net
./build/kontango enroll https://join.kontango.net --no-tui
```
...
2. Visit `https://docs.kontango.net` for technical docs
3. Fork `https://github.com/KontangoOSS/TangoKore`
4. Build SDKs in your language of choice

**For Operations:**
1. Deploy join endpoint at `join.kontango.net`
